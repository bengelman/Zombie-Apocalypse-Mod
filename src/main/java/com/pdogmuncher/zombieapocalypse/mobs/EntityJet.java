...
public class EntityJet extends EntityAnimal
{
...
public EntityJet(World par1World)
{
	 super(par1World);
...
public EntityJet(World par1World, double par2, double par4, double par6)
{
	 this(par1World);
...
public void onUpdate()
{
	 ...
	 if (this.riddenByEntity != null && !worldObj.isRemote && Keyboard.isKeyDown(Keyboard.KEY_D) && !this.isAirBorne){
		 this.fuel--;
		 
		 this.setVelocity(this.riddenByEntity.getLookVec().rotateYaw((float) -(Math.PI / 2)).xCoord / 2, this.motionY, this.riddenByEntity.getLookVec().rotateYaw((float) -(Math.PI / 2)).zCoord / 2);
	 ...
	 if (this.riddenByEntity != null && !worldObj.isRemote && Keyboard.isKeyDown(Keyboard.KEY_A) && !this.isAirBorne){
		 this.fuel--;
		 this.setVelocity(this.riddenByEntity.getLookVec().rotateYaw((float) (Math.PI / 2)).xCoord / 2, this.motionY, this.riddenByEntity.getLookVec().rotateYaw((float) (Math.PI / 2)).zCoord / 2);
		 if(isCollidedHorizontally)
         {
	 ...
	 if (this.riddenByEntity != null && !worldObj.isRemote && Keyboard.isKeyDown(Keyboard.KEY_W)){
		 System.out.println("X look " + this.riddenByEntity.getLookVec().xCoord);
		 this.fuel--;
		 this.rotationYaw = (float) Math.toDegrees(Math.atan2(riddenByEntity.getLookVec().zCoord, riddenByEntity.getLookVec().xCoord));
		 if (Math.abs(this.velocityX) + Math.abs(this.velocityZ) <= 0.5f){
			 if (this.getAge() % 2 == 0){
				 this.setVelocity(this.velocityX + this.riddenByEntity.getLookVec().xCoord / 10, this.velocityY, this.velocityZ + this.riddenByEntity.getLookVec().zCoord / 10);
			 }
			 
		 }
		 else{
			 this.setVelocity(this.riddenByEntity.getLookVec().xCoord / 2, this.velocityY, this.riddenByEntity.getLookVec().zCoord / 2);
		 }
		 this.lastVelX = this.velocityX;
		 this.lastVelZ = this.velocityZ;
		 System.out.println("X velocity: " + this.velocityX + " Z velocity: " + this.velocityZ);
		 //this.setVelocity(this.riddenByEntity.getLookVec().xCoord / 2, this.motionY, this.riddenByEntity.getLookVec().zCoord / 2);
		 if(isCollidedHorizontally)
         {
	 ...
	 if (Keyboard.isKeyDown(Keyboard.KEY_W)){
		 ...
		 this.velocityY = -0.1;
		 this.setVelocity(lastVelX, velocityY, lastVelZ);
	 }
	 /*

...
public class EntityMobTransporter extends EntityAnimal
{
...
public EntityMobTransporter(World par1World)
{
	 super(par1World);
...
public void damageEntity(DamageSource source, float amount){
	...
	if (this.getHealth() < 1){
		this.dropItem(ZombieApocalypse.mob_transporter, 1);
	}
}
/**
...
public EntityMobTransporter(World par1World, double par2, double par4, double par6)
{
	 this(par1World);
...
public void onUpdate()
{
	 ...
	 if (this.riddenByEntity == null){
		 //AxisAlignedBB.fromBounds(this.posX, this.posY, this.posZ, this.posX + 1.5f, this.posY + 1, this.posZ + 1.5f)
		 if (this == null || this.worldObj == null){
			 return;
		 }
		 List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().expand(0.5f, 0, 0.5f));
		 if (!list.isEmpty()){
			 for (int i = 0; i < list.size(); i++){
				 if (list.get(i) instanceof EntityPlayer || !(list.get(i) instanceof EntityCreature)){
					 
				 }
				 else if (list.get(i) instanceof EntityMobTransporter || list.get(i) instanceof EntityCar || list.get(i) instanceof EntityTank){
					 
				 }
				 else{
					 ((Entity)list.get(0)).mountEntity(this);
					 break;
				 }
			 }
...
{
	this.addVelocity(par1EntityPlayer.getLookVec().xCoord / 2, 0, par1EntityPlayer.getLookVec().zCoord / 2);
	return true;
}
/**

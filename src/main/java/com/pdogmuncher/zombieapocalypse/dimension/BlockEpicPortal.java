...
public class BlockEpicPortal extends BlockPortal
{
    ...
    {
        BlockEpicPortal.Size size = new BlockEpicPortal.Size(worldIn, p_176548_2_, EnumFacing.Axis.X);
        ...
        {
            BlockEpicPortal.Size size1 = new BlockEpicPortal.Size(worldIn, p_176548_2_, EnumFacing.Axis.Z);
    ...
    public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock)
    {
        ...
        BlockEpicPortal.Size size;

        if (axis == EnumFacing.Axis.X)
        {
            size = new BlockEpicPortal.Size(worldIn, pos, EnumFacing.Axis.X);
        ...
        {
            size = new BlockEpicPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
    ...
    {
        if (entityIn.ridingEntity == null && entityIn.riddenByEntity == null && entityIn instanceof EntityPlayer && !worldIn.isRemote)
        {
        	EntityPlayerMP player = (EntityPlayerMP) entityIn;
        	if (player.getCurrentEquippedItem() == null){
        		//player.addChatMessage(new ChatComponentText("Use a Portal Entrance/Exit Key!"));
        	}
        	else if (player.dimension != 3 && player.getCurrentEquippedItem().getItem() == ZombieApocalypse.entrance_key){
        		//player.mcServer.getConfigurationManager().transferPlayerToDimension(player, 3, (Teleporter)new EpicTeleporter(player.mcServer.worldServerForDimension(3)));
        		//player.setPosition(player.posX + 1, player.posY, player.posZ + 1);
        	}
        	else if (player.dimension == 3 && player.getCurrentEquippedItem().getItem() == ZombieApocalypse.exit_key){
        		player.mcServer.getConfigurationManager().transferPlayerToDimension(player, 0, (Teleporter)new EpicTeleporter(player.mcServer.worldServerForDimension(0)));
        		//player.setPosition(player.posX + 1, player.posY, player.posZ + 1);
        	}
        	else{
        		//player.addChatMessage(new ChatComponentText("Use a Portal Entrance/Exit Key!"));
    ...
    public static class Size
        {
            ...
            protected int func_180120_a(BlockPos p_180120_1_, EnumFacing p_180120_2_)
            {
                ...
                for (i = 0; i < 22; ++i)
                {
                    ...
                    if (!this.func_150857_a(this.world.getBlockState(blockpos1).getBlock()) || this.world.getBlockState(blockpos1.down()).getBlock() != Blocks.bedrock)
                    {
                        break;
                    }
                }
                ...
                return block == Blocks.bedrock ? i : 0;
            }
            ...
            protected int func_150858_a()
            {
                ...
                for (this.field_150862_g = 0; this.field_150862_g < 21; ++this.field_150862_g)
                {
                    for (i = 0; i < this.field_150868_h; ++i)
                    {
                        ...
                        if (block == ZombieApocalypse.epicPortal)
                        {
                            ++this.field_150864_e;
                        }
                ...
                for (i = 0; i < this.field_150868_h; ++i)
                {
                    if (this.world.getBlockState(this.field_150861_f.offset(this.field_150866_c, i).up(this.field_150862_g)).getBlock() != Blocks.bedrock)
                    {
            ...
            {
                return p_150857_1_.getMaterial() == Material.air || p_150857_1_ == Blocks.fire || p_150857_1_ == ZombieApocalypse.epicPortal;
            }
            ...
            public void func_150859_c()
            {
                for (int i = 0; i < this.field_150868_h; ++i)
                {
                    ...
                    {
                        this.world.setBlockState(blockpos.up(j), ZombieApocalypse.epicPortal.getDefaultState().withProperty(BlockEpicPortal.AXIS, this.axis), 2);
                    }
                }
            }
        }

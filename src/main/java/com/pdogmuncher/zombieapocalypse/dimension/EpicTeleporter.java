...
public class EpicTeleporter extends Teleporter
{
    ...
    public EpicTeleporter(WorldServer worldIn)
    {
    	super (worldIn);
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? Blocks.bedrock.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
        ...
        {
            EpicTeleporter.PortalPosition portalposition = (EpicTeleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(k);
            d0 = 0.0D;
            ...
            for (int l = -128; l <= 128; ++l)
            {
                ...
                for (int i1 = -128; i1 <= 128; ++i1)
                {
                    for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                    {
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos).getBlock() == ZombieApocalypse.epicPortal)
                        {
                            while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == ZombieApocalypse.epicPortal)
                            {
                                blockpos = blockpos1;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag1)
            {
                this.destinationCoordinateCache.add(k, new EpicTeleporter.PortalPosition((BlockPos)object, this.worldServerInstance.getTotalWorldTime()));
            ...
            if (this.worldServerInstance.getBlockState(((BlockPos)object).west()).getBlock() == ZombieApocalypse.epicPortal)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).east()).getBlock() ==ZombieApocalypse.epicPortal)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).north()).getBlock() == ZombieApocalypse.epicPortal)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).south()).getBlock() == ZombieApocalypse.epicPortal)
            {
    ...
    {
    	if (true){
    		BlockPos pos = p_85188_1_.getPosition();
    		...
    		if (p_85188_1_ instanceof EntityPlayer){
    			EntityPlayer player = (EntityPlayer) p_85188_1_;
    			player.inventory.addItemStackToInventory(new ItemStack(ZombieApocalypse.dragon_flute));
    		}
            ...
            for (i3 = -1; i3 <= 1; ++i3)
            {
                for (j3 = 1; j3 < 3; ++j3)
                {
                    for (k3 = -1; k3 < 3; ++k3)
                    {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        boolean flag = k3 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(l3, i4, j4), flag ? Blocks.bedrock.getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
        }

        IBlockState iblockstate = ZombieApocalypse.epicPortal.getDefaultState().withProperty(BlockPortal.AXIS, l5 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
        ...
        for (j3 = 0; j3 < 4; ++j3)
        {
            for (k3 = 0; k3 < 4; ++k3)
            {
                for (l3 = -1; l3 < 4; ++l3)
                {
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.bedrock.getDefaultState() : iblockstate, 2);
                }
            }
    ...
    public void removeStalePortalLocations(long p_85189_1_)
    {
        if (p_85189_1_ % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                EpicTeleporter.PortalPosition portalposition = (EpicTeleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
    ...
    public boolean makePortal(BlockPos p_85188_1_)
    {
    	
        byte b0 = 16;
        double d0 = -1.0D;
        int i = MathHelper.floor_double(p_85188_1_.getX());
        int j = MathHelper.floor_double(p_85188_1_.getY());
        int k = MathHelper.floor_double(p_85188_1_.getZ());
        int l = i;
        int i1 = j;
        int j1 = k;
        int k1 = 0;
        ...
        for (i2 = i - b0; i2 <= i + b0; ++i2)
        {
            d1 = (double)i2 + 0.5D - p_85188_1_.getX();

            for (k2 = k - b0; k2 <= k + b0; ++k2)
            {
                d2 = (double)k2 + 0.5D - p_85188_1_.getZ();
                label271:
                ...
                for (i3 = this.worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3)
                {
                    if (this.worldServerInstance.isAirBlock(new BlockPos(i2, i3, k2)))
                    {
                        ...
                        for (j3 = l1; j3 < l1 + 4; ++j3)
                        {
                            ...
                            d3 = (double)i3 + 0.5D - p_85188_1_.getY();
                            d4 = d1 * d1 + d3 * d3 + d2 * d2;
